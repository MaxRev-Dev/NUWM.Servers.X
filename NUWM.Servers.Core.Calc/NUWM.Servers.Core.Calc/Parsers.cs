...
using System.Web;
using static JSON.SpecialtiesVisualiser.BranchSpecialLinqer;
        ...
        public partial class Specialty
        {
            ...
            public class SpecialtyParser
            {
                ...
                public static async void Run()
                {
                    ...
                    NUWM.Servers.Core.Calc.MainApp.taskDelayH = int.Parse(new Regex(@"(?<=delayH\:)[0-9]*").Match(direct).Groups[0].Value);
                    GC.Collect();
                }
                async public void GetLastModulus()
                {
                    ...
                    try {
                        HttpResponseMessage s = await (new CreateClientRequest(requestUri.AbsoluteUri).GetAsync());
                        ...
                        Logger.Current.Errors.Add(ex);
                        await MainApp.Current.LoadCache();
                    }
                }
                static async void GetPage(Object l)
                {
                    ...
                    if (linker != "null") {
                        ...
                        var link = sitesUrl + linker;
                        CreateClientRequest request = new CreateClientRequest(link);
                        try {
                            ...
                            Logger.Current.Errors.Add(new Exception(
                               string.Join('\n',new object[] {
                                   new string('-',20),
                                   param[0],HtmlEntity.DeEntitize(el.InnerText).Trim(' '),
                                   link,
                                   string.Format("Item {0} fetch error: ",item) + ex.Message,
                               new string('-',20)})));
                        }
                    }
                }
                ...
                static void ParsePage(Object paged)
                {
                    ...
                    try {
                        ...
                        MainApp.CurrentParser.res.Add(sp);
                    } catch (Exception ex) {
                ...
                {
                    if (MainApp.CurrentParser.res.Count == 0) return new List<string>();
                    List<string> dist = new List<string>();
                    foreach (var t in MainApp.CurrentParser.res) {
