(function () {
    ...
    var id = 0,
        ...
        base = ".",
        lk = {
            zc_css: base + "/ZNOCalcs/css/nuwm-zno.calcs.min.css",
            hlp: base + "/api/calc?hlp=",
            anim_css: base + "/ZNOCalcs/css/animate.css",
        ...
        loc = {
            ...
            ru: {
                ...
                js_prior_ph: 'Приоритет заявления 1 - 7',
            ...
            en: {
                ...
                js_open: 'Read full',
                js_nodata: 'N/A',
                js_tp_200: 'Plea priority - from 1 to 7',
    ...
    function popovers() {
        setTimeout(function () {
            ...
            jQuery('.zc_your_mark').attr('title', t.js_your_mark);
            jQuery('.zc_last_mark').attr('title', t.js_last_mark);
            jQuery('.zc_your_mark').popover('dispose');
            jQuery('.zc_last_mark').popover('dispose');
            jQuery(function () {
    ...
    function loadjscssfile(t, s) {
        ...
        var timerId = setInterval(function () {
            if (css1 && css2 && !reqsent) {
                reqsent = true;
                jQuery.ajax({
                    url: base + "/ZNOCalcs/zno-calcs-misc.html",
    ...
    function CreateElement(t) {
        ...
        r.setAttribute("class", "card my-3"), r.innerHTML = "<div class='card-header p-0' role='tab' id='zc_data_heading" + id + "'> <div class='zno-calcs_sp-data my-2 mt-4'>  <p class='zno-calcs_sp-data_title'> <strong>" + s + "</strong></p>" + (null == a ? "" : "  <p class='zno-calcs_sp-data_subtitle'>" + a + "</p>") +
    ...
    function LoadCards() {
        ...
        for (var i = 1; i <= 3; i++) {
            if (jQuery('#zc-s' + i + '-val').val() === "" ||
                jQuery('#zc-s' + i).text() === loc[getVault()].js_chose ||
                jQuery('#zc-AverAt').val() === ""
            ) {
                if (window.localStorage) {
        ...
        setTimeout(function () {
            jQuery.ajax({
                url: base + "/api/calc?n=" + all_names + '&v=' + all_values + '&vl=' + vill + (!!pr ? '&pr=' + pr : '') + (!!avm ? '&avm=' + avm : '')
            }).done(function(t){
                 SetObjects(t);
    ...
    function ShowCls() {
        var t = $('.zc_clear').length;
        if (t == 0) {
            var el = document.createElement("button");
            el.setAttribute("type", "button");
            el.setAttribute("class", "btn  btn-primary  zc_clear my-2");
            el.textContent = 'X';
            $('#zc_sub_wrap').append(el);
        }
    }

    function ClearAll() {
        jQuery(".zc_clear").remove();
        jQuery.ajax('../../api/calc?hlp=').done(function (data) {
            var odata = data.response.split(',');
            var lost = "Українська";
            lost = odata.filter(function (item) {
                return item.includes(lost)
            });

            jQuery('#zc-s1').text(lost);
            jQuery('#zc-s2').text(loc[getVault()].js_chose);
            jQuery('#zc-s3').text(loc[getVault()].js_chose);
            jQuery('input').val('');
            splitForDrops(odata.filter(function (item) {
                return item !== lost[0]
            }).join(','));


            jQuery(".card.animated").addClass("fadeOut");
            setTimeout(function () {
                jQuery(".card.animated").remove();
            }, 1200);
        });
    ...
    jQuery(document).ready(function () {
        ...
        jQuery(document).on("click", ".zc_clear", function () {
            ClearAll()
        }),
